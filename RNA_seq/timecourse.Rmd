...
## Both
```{r}
top_time_fb <- top_time %>% 
  mutate(gene_name = rownames(.)) %>% 
  left_join(gene_names_fb)

time_KEGG_enrichment <- vector("list")
id_list <- vector("list")

top_time_KEGG <- top_time_fb %>% 
  mutate(direction_2 = case_when(Hr_2.UC <0 ~ "2h_neg",
                                 Hr_2.UC >0 ~ "2h_pos"),
         direction_4 = case_when(Hr_4.UC <0 ~ "4h_neg",
                                 Hr_4.UC >0 ~ "4h_pos")) %>% 
  mutate(direction = paste(direction_2, direction_4)) %>% 
  filter(!direction %in% c("NA NA", "2h_neg NA", "NA 4h_pos"))
 
directions <- unique(top_time_KEGG$direction)

for(i in directions){
  genes <- top_time_KEGG %>% 
    filter(direction == i & adj.P.Val < 0.05)

original_gene_list <- genes$Hr_2.UC
...
kk <- enrichKEGG(gene = gene,
                 ...
                 pvalueCutoff = 0.05)
id_list[[i]] <- ids
time_KEGG_enrichment[[i]] <- kk
}

downregulated_KEGG <- time_KEGG_enrichment$`2h_neg 4h_neg`@result
apoptosis_down <- downregulated_KEGG %>% filter(Description == "Apoptosis - fly")
upregulated_KEGG <- time_KEGG_enrichment$`2h_pos 4h_pos`@result
toll_imd_up <- upregulated_KEGG %>% filter(Description == "Toll and Imd signaling pathway")


time_KEGG_enrichment_2 <- vector("list")
id_list_2 <- vector("list")
directions <- unique(top_time_KEGG$direction_2)

for(i in directions){
  genes <- top_time_KEGG %>% 
    filter(direction_2 == i & P.Value < 0.05)
...
kk <- enrichKEGG(gene = gene,
                 ...
                 pvalueCutoff = 0.05)
id_list_2[[i]] <- ids
time_KEGG_enrichment_2[[i]] <- kk
}

(downregulated_dotplot <- dotplot(time_KEGG_enrichment$`2h_neg 4h_neg`) + theme(aspect.ratio = 1) + ggtitle("KEGG pathways enriched in downregulated genes"))
#ggsave(filename = "results/downregulated_kegg.png", downregulated_dotplot, width = 7, height = 5)

(upregulated_dotplot_2 <- dotplot(time_KEGG_enrichment$`2h_pos 4h_pos`) + theme(aspect.ratio = 1) + ggtitle("KEGG pathways enriched in upregulated genes"))
#ggsave(filename = "results/upregulated_kegg_2.png", upregulated_dotplot_2, width = 7, height = 5)
...
(toll_imd_vol_4_plot <- ggplot(toll_imd_vol, aes(x = Hr_4.UC, y = -log10(P.Value), label = gene_name)) +
    geom_point(aes(color = color_4)) +
...
(endocytosis_plot <- ggplot(gg_endocytosis, aes(x = time_hours, y = count, color = gene, fill = gene)) +
  ...
  theme_bw() +
  theme(legend.position = "none", aspect.ratio = 1) +
  facet_grid(~Sex, labeller = labeller(direction = direction_labs, Sex = sex_labs)) +
  ylab("Gene counts (normalized to zero)") +
  xlab("Hours post injection") +
  ggtitle("Expression of endocytosis pathway in hemocytes") +
  theme(aspect.ratio = 1, strip.background = element_rect(fill = "white")))
#ggsave("results/endocytosis_plot.png", plot = endocytosis_plot, width = 5, height = 3) 
...
(phagosome_plot <- ggplot(gg_phagosome, aes(x = time_hours, y = count, color = gene, fill = gene)) +
  ...
  theme(aspect.ratio = 1, strip.background = element_rect(fill = "white")))
#ggsave("Results/phagosome_plot.png", plot = phagosome_plot, width = 5, height = 3) 
```
...
(module_heatmap <- mME %>% 
  filter(name %in% c("yellow")) %>% 
    mutate(treatment = gsub("_hem","", treatment))  %>% 
  mutate(name = factor(name, levels = c("yellow")),
         treatment = factor(treatment, levels = c("F_UC_2", "F_UC_3", "F_UC_4", "F_UC_5", "M_UC_2", "M_UC_3", "M_UC_5", "F_2h_1", "F_2h_2", "F_2h_3", "F_2h_4", "F_2h_5", "M_2h_1", "M_2h_2", "M_2h_3", "M_2h_4", "M_2h_5", "F_4h_1", "F_4h_2", "F_4h_3", "F_4h_4", "F_4h_5", "M_4h_1", "M_4h_2", "M_4h_3", "M_4h_4", "M_4h_5")))%>% 
  ...
  annotate("segment", x = 0.5, xend = 7.5, y = 1.7, yend = 1.7, colour = "#000004", linewidth = 5) +
  annotate("segment", x = 7.5, xend = 17.5, y = 1.7, yend = 1.7, colour = "#FEC287", linewidth = 5) +
  annotate("segment", x = 17.5, xend = 27.5, y = 1.7, yend = 1.7, colour = "#F56B5C", linewidth = 5) +
  annotate("segment", x = 0.5, xend = 4.5, y = 1.6, yend = 1.6, colour = "#D3C164", linewidth = 5) +
  annotate("segment", x = 4.5, xend = 7.5, y = 1.6, yend = 1.6, colour = "#05366E", linewidth = 5) +
  annotate("segment", x = 7.5, xend = 12.5, y = 1.6, yend = 1.6, colour = "#D3C164", linewidth = 5) +
  annotate("segment", x = 12.5, xend = 17.5, y = 1.6, yend = 1.6, colour = "#05366E", linewidth = 5) +
  annotate("segment", x = 17.5, xend = 22.5, y = 1.6, yend = 1.6, colour = "#D3C164", linewidth = 5) +
  annotate("segment", x = 22.5, xend = 27.5, y = 1.6, yend = 1.6, colour = "#05366E", linewidth = 5) +
  expand_limits(y = 1.75))
ggsave("results/module_heatmap.png", module_heatmap, height = 2.5, width = 7.5)
```
...
(normalized_ex_yellow <- submod_df%>%
  filter(module %in% c("yellow")) %>% 
  mutate(name = factor(name, levels = c("F_UC_hem_2", "F_UC_hem_3", "F_UC_hem_4", "F_UC_hem_5", "M_UC_hem_2", "M_UC_hem_3", "M_UC_hem_5", "F_2h_hem_1", "F_2h_hem_2", "F_2h_hem_3", "F_2h_hem_4", "F_2h_hem_5", "M_2h_hem_1", "M_2h_hem_2", "M_2h_hem_3", "M_2h_hem_4", "M_2h_hem_5", "F_4h_hem_1", "F_4h_hem_2", "F_4h_hem_3", "F_4h_hem_4", "F_4h_hem_5", "M_4h_hem_1", "M_4h_hem_2", "M_4h_hem_3", "M_4h_hem_4", "M_4h_hem_5"))) %>% 
  ...
  scale_color_manual(values = c("#e6b800")) +
  #facet_grid(rows = vars(module)) +
  labs(x = "Treatment",
       y = "Normalized\nexpression") +
  annotate("segment", x = 1, xend = 7.5, y = 17, yend = 17, colour = "#000004", linewidth = 2) +
  annotate("segment", x = 7.5, xend = 17.5, y = 17, yend = 17, colour = "#FEC287", linewidth = 2) +
  annotate("segment", x = 17.5, xend = 27, y = 17, yend = 17, colour = "#F56B5C", linewidth = 2) +
  annotate("segment", x = 1, xend = 4.5, y = 18, yend = 18, colour = "#D3C164", linewidth = 2) +
  annotate("segment", x = 4.5, xend = 7.5, y = 18, yend = 18, colour = "#05366E", linewidth = 2) +
  annotate("segment", x = 7.5, xend = 12.5, y = 18, yend = 18, colour = "#D3C164", linewidth = 2) +
  annotate("segment", x = 12.5, xend = 17.5, y = 18, yend = 18, colour = "#05366E", linewidth = 2) +
  annotate("segment", x = 17.5, xend = 22.5, y = 18, yend = 18, colour = "#D3C164", linewidth = 2) +
  annotate("segment", x = 22.5, xend = 27, y = 18, yend = 18, colour = "#05366E", linewidth = 2) )

#ggsave("results/normalized_ex_yellow.png", normalized_ex_yellow, height = 2, width = 7.5)
```
...
for(i in modules){
  ...
  dotplot_graph <- dotplot(kk) +
    theme(aspect.ratio = 1) +
    ggtitle(paste("KEGG enrichment in", i, "module"))
  dotplots[[i]] <- dotplot_graph
  if(nrow(dotplot_graph[["data"]]) > 0){
  ggsave(filename = paste("results/",i, ".png", sep = ""), dotplot_graph, width = 7, height = 5)}
}
```
